      ...
      steps {
        container('git') {
          withCredentials([usernamePassword(credentialsId: 'github-token', usernameVariable: 'GIT_USERNAME', passwordVariable: 'GIT_PAT')]) {
            sh '''
              rm -rf helm-repo
              git clone -b lesson-8-9 https://$GIT_USERNAME:$GIT_PAT@${HELM_REPO_URL#https://} helm-repo
              ...
              git add values.yaml
              git commit -m "Update image tag to $IMAGE_TAG" || echo "Image tag already up to date"
              git push https://$GIT_USERNAME:$GIT_PAT@${HELM_REPO_URL#https://} lesson-8-9
            '''
          }
        }
      }
